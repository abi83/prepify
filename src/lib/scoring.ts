...
export function isAnswerCorrect(q: Question, a: AnswerState): boolean {
  switch (q.type) {
    case 'single_choice': {
      const c = (q.content as unknown as { answers: { id: string; is_correct: boolean }[] }).answers
    ...
    case 'fill_the_gap': {
      const { gaps } = q.content as unknown as { gaps: { index: number; correct_answer_id: string }[] }
      return gaps.every(g => a.fill[g.index - 1] === g.correct_answer_id)
    }
    case 'sorting': {
      const { answers } = q.content as unknown as { answers: { id: string; correct_index: number }[] }
