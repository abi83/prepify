...
describe("computeCost", () => {
  it("prices prompt + cached tokens at the input rate, completion tokens at the output rate", () => {
    const cost = computeCost({ promptTokens: 1_000_000, cachedTokens: 0, completionTokens: 0 }, "gpt-5-nano")
    expect(cost).toBeCloseTo(1.10)
  })

  it("folds cached tokens into the input rate", () => {
    const withoutCache = computeCost({ promptTokens: 1_000_000, cachedTokens: 0, completionTokens: 0 }, "gpt-5-nano")
    const withCache = computeCost({ promptTokens: 500_000, cachedTokens: 500_000, completionTokens: 0 }, "gpt-5-nano")
